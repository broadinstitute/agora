...
class EntityCopySpec extends ApiServiceSpec with FlatSpecLike {
  ...
  it should "reject copying a configuration" in {
    ...
    Post(targetUri, AgoraEntity()) ~>
      ...
      (handleExceptions(ApiService.exceptionHandler) & handleRejections(ApiService.rejectionHandler)) { configurationsService.querySingleRoute } ~> check {
        assert(!handled)
    }
  }
  ...
  private def testEntityCopy(expectedStatus: StatusCode, asUser: String,
    ...
    Post(postUri, AgoraEntity()) ~>
      ...
      (handleExceptions(ApiService.exceptionHandler) & handleRejections(ApiService.rejectionHandler)) { methodsService.querySingleRoute } ~> check {
